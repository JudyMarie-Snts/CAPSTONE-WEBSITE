...
export default function RefillRequest() {
  ...
  console.log('🔍 Timer sources:', { minutesParam, storedDuration, defaultWillBe: 90 * 60 })
    
    // Clear any old global localStorage duration to use new default
    if (storedDuration && storedDuration !== 90 * 60) {
  ...
  const defaultDuration = 90 * 60 // 1 hour 30 minutes in seconds
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      if (res?.ok && res?.data?.success) {
        ...
        // Use navigate instead of window.location.href for better SPA routing
        navigate(`/refill-request-submitted${refillId ? `?id=${encodeURIComponent(refillId)}` : ''}`)
        return
      } else {
        // Better error handling
        const errorMsg = res?.data?.message || res?.data?.error || res?.error || res?.message || 'Failed to submit refill request. Please try again.'
