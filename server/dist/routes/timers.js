...
router.post('/', async (req, res) => {
    try {
        const { customer_name, table_id, table_code, order_id, start_time } = req.body;
        ...
        const [existingRows] = await database_1.pool.execute('SELECT id FROM customer_timers WHERE table_id = ? AND is_active = 1', [finalTableId]);
        if (existingRows.length > 0) {
            // Update existing timer
            const [result] = await database_1.pool.execute(`UPDATE customer_timers 
         SET customer_name = ?, order_id = ?, start_time = COALESCE(?, start_time), updated_at = NOW()
         WHERE id = ?`, [customer_name, order_id || null, start_time || null, existingRows[0].id]);
            // Fetch updated timer
        ...
        WHERE ct.id = ?`, [existingRows[0].id]);
            ...
            // Create new timer
            const [result] = await database_1.pool.execute(`INSERT INTO customer_timers 
          (customer_name, table_id, order_id, start_time, is_active, created_at, updated_at)
        VALUES (?, ?, ?, COALESCE(?, NOW()), 1, NOW(), NOW())`, [customer_name, finalTableId, order_id || null, start_time || null]);
            // Fetch created timer
            const [newRows] = await database_1.pool.execute(`SELECT 
          ct.*,
          rt.table_number,
          rt.table_code,
          o.order_code
...
router.patch('/:id', async (req, res) => {
    try {
        ...
        const [existingRows] = await database_1.pool.execute('SELECT * FROM customer_timers WHERE id = ?', [id]);
        if (existingRows.length === 0) {
            return res.status(404).json({
                success: false,
        ...
        await database_1.pool.execute(`UPDATE customer_timers SET ${updateFields.join(', ')} WHERE id = ?`, updateParams);
        // Fetch updated timer
        const [updatedRows] = await database_1.pool.execute(`SELECT 
        ct.*,
        rt.table_number,
...
router.patch('/table/:tableCode/stop', async (req, res) => {
    try {
        ...
        const [tableRows] = await database_1.pool.execute('SELECT id FROM restaurant_tables WHERE table_code = ?', [tableCode]);
        if (tableRows.length === 0) {
            return res.status(404).json({
                success: false,
                message: 'Table not found with the provided code'
