...
router.post('/', async (req, res) => {
  try {
    ...
    const [result] = await pool.execute<ResultSetHeader>(
      ...
      VALUES (?, ?, ?, 'pending', ?, ?, NOW())`,
      [table_code, table_id, customer_id || null, request_type || null, notes || null]
    );
    ...
    try {
      ...
      const [existingTimerRows] = await pool.execute<RowDataPacket[]>(
      ...
      if (existingTimerRows.length > 0) {
        ...
        await pool.execute(
          `UPDATE customer_timers 
           ...
           WHERE id = ?`,
          [customerName, existingTimerRows[0].id]
        );
    ...
    const [newRequest] = await pool.execute<RowDataPacket[]>(
      `SELECT 
        rr.*,
        rt.table_number,
        c.first_name,
...
router.patch('/:id/status', async (req, res) => {
  try {
    ...
    await pool.execute(
      `UPDATE refill_requests SET ${updateFields.join(', ')} WHERE id = ?`,
      updateParams
    );
    ...
    try {
      ...
      if (status === 'completed') {
        ...
        const [activeTimerRows] = await pool.execute<RowDataPacket[]>(
          'SELECT id FROM customer_timers WHERE table_id = ? AND is_active = 1',
          [tableId]
        );
...
router.delete('/:id', async (req, res) => {
  try {
    ...
    await pool.execute('DELETE FROM refill_requests WHERE id = ?', [id]);
...
router.post('/validate-table', async (req, res) => {
  try {
    ...
    const [rows] = await pool.execute<RowDataPacket[]>(
      'SELECT id, table_number, table_code, status FROM restaurant_tables WHERE table_code = ?',
      [table_code]
    );
